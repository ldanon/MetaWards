...
dataIR %>%
  ...
  geom_line(aes(y=Incidence),alpha=0.1)+
...
dataUV %>%
  filter(Time<500) %>% 
  ggplot(aes(x=Time,colour=Seasonal))+
  geom_line(aes(y=Incidence),alpha=1,size=2)+
#  geom_line(aes(y=Recovered),alpha=0.1)+
  theme_minimal_grid()+
#  theme(legend.position = "none")+
 #scale_y_continuous(labels = unit_format(unit = "M"))+
...
dataUV %>% 
  group_by(UV) %>% 
  summarise(which.max(Incidence),100*max(Recovered)/56082077)
#->p
#ggplotly(p)
